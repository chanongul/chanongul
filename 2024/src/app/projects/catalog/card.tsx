...
export default function ProjectsPageCard({
  ...
  return (
    <div className="col-span-full size-full sm:col-span-3 xl:col-span-2">
      <div className="col-span-full flex size-full flex-col gap-4 sm:col-span-3 xl:col-span-2">
        <Link
          href={`/projects/${slug}`}
          className="group peer relative block aspect-[3/2] w-full overflow-hidden rounded-2xl border border-neutral-200 bg-white transition-colors duration-300 dark:border-neutral-800 dark:bg-charcoal"
        >
          <Image
            src={preview}
            alt={name}
            width={1000}
            height={1000}
            className="absolute transition-transform duration-1000 touch:group-active:scale-110 no-touch:group-hover:scale-110"
          />
        ...
        <div className="flex items-center justify-between gap-2 text-[1em] sm:text-[1.05em] touch:peer-active:[&>div>a]:after:w-full no-touch:peer-hover:[&>div>a]:after:w-full">
          <div className="flex flex-col gap-2">
            <Link
              href={`/projects/${slug}`}
              className="relative w-max text-[1.45em] font-medium leading-none transition-colors duration-300 after:absolute after:top-[110%] after:block after:h-px after:w-0 after:bg-black after:transition-all after:duration-500 touch:active:after:w-full no-touch:hover:after:w-full dark:after:bg-white"
            >
              {name}
            </Link>
