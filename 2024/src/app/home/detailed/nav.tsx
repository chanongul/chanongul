...
export default function Navigator({ currentSection }: DetailedHomePageProps) {
  ...
  return (
    <>
      <button
        className={`absolute top-10 -translate-y-1/2 -translate-x-1/2 left-10 transition-all duration-300 text-black dark:text-white ${curSect ? "opacity-100 delay-200" : "opacity-0"}`}
        onClick={() => setCurSect(null)}
      >
        <IoClose />
      </button>
      <nav
        className={`z-50 absolute top-10 left-1/2 -translate-x-1/2 -translate-y-1/2 transition-all duration-300 text-black dark:text-white flex gap-4 ${curSect ? "opacity-100 delay-200" : "opacity-0"}`}
      >
